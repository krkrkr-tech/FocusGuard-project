...
from db import (
    ...
    get_all_users, delete_user, create_teacher_by_admin, create_student_by_admin, create_admin_by_admin, check_email_exists,
...
def admin_page():
    ...
    tab_teachers, tab_students, tab_admins, tab_history, tab_stats = st.tabs(["👨‍🏫 Teachers", "🎓 Students", "⚙️ Admins", "📋 History", "📊 Statistics"])
    ...
    with tab_students:
        ...
        st.divider()
        st.subheader("➕ Create Student Directly")
        st.caption("This bypasses email verification - student can login immediately")
        with st.form("add_student_form"):
            ...
            student_pass = st.text_input("Password", type="password", placeholder="At least 6 characters")
            
            if st.form_submit_button("Create Student (Immediate Access)", type="primary", use_container_width=True):
                if not student_name or not student_email or not student_pass:
                    st.error("Fill in all fields")
                elif len(student_pass) < 6:
                    st.error("Password must be at least 6 characters")
                else:
                    result = create_student_by_admin(student_email, student_pass, student_name)
                    if result["success"]:
                        st.success(f"✅ Student {student_name} created and can login now")
                    else:
                        st.error(f"❌ {result['message']}")
    
    with tab_admins:
        st.subheader("⚙️ Admin Management")
        st.caption("Create admins with different permission levels")
        
        # Admin types explanation
        admin_info = {
            "super": "🔴 Full Access - Can manage all users, exams, and settings",
            "email_checker": "🟡 Limited - Can only view student/teacher emails",
            "exam_manager": "🟢 Limited - Can manage exams and assignments",
            "student_manager": "🔵 Limited - Can manage student accounts only"
        }
        
        st.markdown("**Admin Types:**")
        for admin_type, description in admin_info.items():
        ...
        with st.form("add_admin_form"):
            admin_name = st.text_input("Full Name", placeholder="Admin Name")
            admin_email = st.text_input("Email", placeholder="admin@example.com")
            admin_pass = st.text_input("Password", type="password", placeholder="At least 6 characters")
            admin_type = st.selectbox(
                "Admin Level",
                options=["super", "email_checker", "exam_manager", "student_manager"],
                format_func=lambda x: admin_info[x]
            )
            ...
            if st.form_submit_button("Create Admin", type="primary", use_container_width=True):
                if not admin_name or not admin_email or not admin_pass:
                    ...
                    result = create_admin_by_admin(admin_email, admin_pass, admin_name, admin_type)
                    if result["success"]:
                        st.success(f"✅ Admin '{admin_name}' ({admin_type}) created successfully")
                    else:
                        st.error(f"❌ {result['message']}")
        ...
        if admins:
            st.caption(f"Total admins: {len(admins)}")
            for admin in admins:
                col1, col2, col3, col4 = st.columns([2, 2, 2, 1.5])
                col1.write(f"**{admin['full_name']}**")
                col2.caption(admin['email'])
                admin_type = admin.get('admin_type', 'unknown')
                col3.caption(f"Type: {admin_type}")
        else:
...
def teacher_page():
    ...
    tab_exams, tab_students = st.tabs(["📋 Exams", "👥 Enrolled Students"])
    ...
    with tab_exams:
        ...
        with st.form("create_exam_form"):
            exam_title = st.text_input("Exam Title", placeholder="Final Mathematics Exam")
            exam_desc = st.text_area("Description", placeholder="Exam details and instructions...", height=80)
            
            if st.form_submit_button("Create Exam", type="primary", use_container_width=True):
                if not exam_title.strip():
                    st.error("Exam title is required")
                else:
                    result = create_exam(exam_title, exam_desc, st.session_state.user_id)
                    if result["success"]:
                        st.success(f"✅ Exam '{exam_title}' created successfully!")
                        st.rerun()
                    else:
                        st.error(f"❌ {result.get('message', 'Error creating exam')}")
    
    with tab_students:
        st.subheader("Enrolled Students")
        all_users = get_all_users()
        verified_students = [u for u in all_users if u['role'] == 'student' and u['is_verified']]
        ...
        if verified_students:
            for user in verified_students:
                ...
                col1.markdown(f"**{user['full_name']}**")
                col2.markdown(f"`{user['email']}`")
                if col3.button("🗑️", key=f"del_enr_{user['id']}", help="Remove student"):
                    if delete_user(user['id']):
